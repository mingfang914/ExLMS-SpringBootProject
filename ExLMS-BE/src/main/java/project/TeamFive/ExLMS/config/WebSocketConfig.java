                        ...
                        try {
                            ...
                            if (userEmail != null) {
                                ...
                                if (jwtService.isTokenValid(jwt, userDetails) && userDetails instanceof User) {
                                    ...
                                    // Quan trọng: Phải dùng ID.toString() làm Name để khớp với convertAndSendToUser
                                    UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(
                                            user, null, userDetails.getAuthorities()
                                    );
                                    
                                    // Customizing the authentication to return UUID as Name
                                    // By default UsernamePasswordAuthenticationToken.getName() returns principal.toString() or principal.username
                                    // Better wrap it or use a token where getName returns what we want
